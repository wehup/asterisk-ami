...
class QueueAddFactory implements FactoryInterface
{
    ...
    public function createResponse(\Guzzle\Http\Message\Response $response)
    {
        ...
        if (preg_match('#^Response: Error\r\nMessage: Unable to add interface: Already there#', $body)) {
            return new Response\QueueInterfaceAlreadyThereResponse();
        }

        if (preg_match('#^Response: Error\r\nMessage: Unable to add interface to queue: No such queue#', $body)) {
